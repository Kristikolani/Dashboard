...
const UserNav = () => {
  ...
  const { setSortOrder } = useUserQueryStore();
  const { sortOrder } = useUserQueryStore((s) => s.userQuery);
  const ref = useRef<HTMLInputElement>(null);
  return (
    <div className="flex w-full justify-between">
      <div className="flex">
        <AuthNav />
        <h1 className=" text-3xl p-2 font-normal text-white ">Users</h1>
        <h5 className=" text-base text-neutral-300 mt-3 p-2">3,702 Total</h5>
      </div>
      <div>
        <div className="flex">
          <div>
